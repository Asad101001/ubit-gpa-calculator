...
- **Serverless "Wrapped" Snapshot**: Generate beautiful, exportable PNG snapshots of your academic performance using Vercel OG image generation.
- **JSON Transcript Export**: Download a structured JSON transcript of your grades.

## Tech Stack
- **Frontend**: React 19 + TypeScript + Vite
- **Serverless API**: Vercel Edge Functions (`@vercel/og`)
...
2. Under "Environment Variables", add the following two keys (used by Vercel Edge Functions):
...
2. Run `npm install`.
3. Create a `.env` file in the root directory with the Supabase variables (see above). These should not be prefixed with VITE_.
